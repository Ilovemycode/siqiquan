...
//    [self.view addSubview:zhuce];
    ...
    UILabel *lanel=[[UILabel alloc]initWithFrame:CGRectMake((self.view.frame.size.width-100)/2, 30, 100, 30)];
    lanel.text=@"思齐圈登录";
...
//        [SVProgressHUD showInfoWithStatus:@"您输入的手机号码格式不正确"];
    ...
    NSString *loginUrl = [NSString stringWithFormat:@"http://api.thinkwit.com/token"];
    
//    NSDictionary *loginDict = @{@"grant_type":@"password",@"username":@"18516605890",@"password":@"123456"};
    
    
    NSDictionary *loginDict = @{@"grant_type":@"password",@"username":user.text,@"password":pwd.text};
    
    NSLog(@"loginDict = %@",loginDict);
    
    
    //    self.window.rootViewController = [[SQ_TabbarController alloc]init];
